...
class classfiler_1(nn.Module):
    def __init__(self, cls_num):
        ...
        self.fc = nn.Sequential(nn.Linear(1280, 640), nn.ReLU(), nn.Linear(640, cls_num))
...
class DSAModule(nn.Module):
    def __init__(self, in_ch):
        ...
        self.dcn1 = nn.Sequential(DeformableConv2d(in_ch, in_ch, 3, padding=1),
                                  nn.GroupNorm(in_ch // 4, in_ch), nn.ReLU())

        self.dcn2 = nn.Sequential(DeformableConv2d(2 * in_ch, in_ch, 3, padding=1),
                                  nn.GroupNorm(in_ch // 4, in_ch), nn.ReLU())
...
class MyModel(nn.Module):
    def __init__(self, cls_num):
        ...
        self.classfer = classfiler_1(cls_num=cls_num)
