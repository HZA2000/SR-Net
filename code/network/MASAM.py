...
class MASAM(nn.Module):
    def __init__(self, num_classes, num_atlas=2, encoder_embed_dim=768, encoder_depth=12, encoder_num_heads=12,
                 encoder_global_attn_indexes=(2, 5, 8, 11), image_size=256, prompt_embed_dim=256, vit_patch_size=16,
                 adapterTrain=True):
        ...
        self.boundary_decoder = MaskDecoder(
                transformer_dim=prompt_embed_dim,
                numClasses=num_classes,
                transformer=TwoWayTransformer(
                    depth=2,
    ...
    def forward(self, img, prompt):
        ...
        mask = self.mask_decoder(image_embeddings=imgFeatures,
                                ...
                                prompt_skip=skip)
        boundary = self.boundary_decoder(image_embeddings=imgFeatures,
                                pe=self.pe(),
                                prompt_embeddings=promptFeatures,
                                prompt_skip=skip)
