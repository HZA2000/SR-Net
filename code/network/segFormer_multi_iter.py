...
class Segformer(SegmentationModel):
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        self.encoder_prompt = get_encoder(
            in_channels=in_channels_prompt,
            depth=encoder_depth,
            weights=encoder_weights,
            **kwargs,
        )
    ...
    def forward(self, x, prompt):
        ...
        features = self.encoder(x)
        features_prompt = self.encoder_prompt(prompt)

        feature_embedding, _ = self.attention(query=features_prompt[-1], key=features[-1], value=features[-1])

        features[-1] = feature_embedding
        decoder_output = self.decoder(*features)
